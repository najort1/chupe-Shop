...
const Principal = ({
  ...
  const fetchProdutos = async (page) => {
    ...
    const response = await axios.get(
      `http://localhost:8080/produtos/listar?page=${page}&size=10`,
      {
  ...
  const handlePesquisa = async (e) => {
    ...
    const response = await axios.get(
      `http://localhost:8080/produtos/listar/nome/?nome=${pesquisa}`,
      {
        validateStatus: function (status) {
          return status <= 500;
        },
      ...
      <main>
        ...
        <div className="sessaoProdutos">
          ...
          <input
            ...
            placeholder="Pesquise por uma chupeta especifica"
            name="pesquisaProduto"
            id=""
            className="pesquisaProduto
          ...
          <section
            className="produtos gap-2 grid grid-cols-3 mt-20
            xl:grid-cols-10
            md:grid-cols-4
          "
          >
              ...
              produtos.map((item, index) => (
                <div className="card" key={index}>
                  ...
                  <div className="card-content bg-gray-300 rounded-md drop-shadow-xl">
                    <h1 className="titulo font-bold text-center dark:text-black">
                      {item.nome}
                    </h1>
                    <p className="preco text-center dark:text-black">
                      R$ {item.preco}
                    </p>
                    <p className="estoque text-center dark:text-black">
